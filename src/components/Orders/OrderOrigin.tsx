...
const OrderOrigin: FC<OrderOriginProps> = ({ setOrder, origin_location, index, handleOriginChange, handleRemoveOrigin, onAddOrigin }) => {
  const origin = origin_location[index] ?? {};
  ...
  const updateAddressFields = (place: google.maps.places.PlaceResult) => {
    ...
    setOrder((prev) => ({
      ...prev,
      origin_location: prev.origin_location.map((loc, i) =>
        i === index
          ? {
              ...loc,
  ...
  const validateAndSetOrigin = useCallback(
    (field: keyof Location, value: string) => {
      ...
      handleOriginChange(index, updatedOrigin);
    },
    [origin, handleOriginChange, index]
  );
  ...
  return (
    <fieldset className="form-section" style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          ...
          <div className="form-group" key={key} style={{ display: 'flex', flexDirection: 'column' }}>
            ...
            {type === 'textarea' ? (
              <textarea
                id={`${key}-${index}`}
                name={key}
                value={(origin[key as keyof Location] as string) || ''}
                onChange={(e) => validateAndSetOrigin(key as keyof Location, e.target.value)}
              ...
              <input
                ...
                name={key}
                type={type}
                value={(origin[key as keyof Location] as string | number) || ''}
                onChange={(e) => validateAndSetOrigin(key as keyof Location, e.target.value)}
                placeholder={placeholder}
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '0px' }}>
        <button type="button" onClick={onAddOrigin} className="add-button">
          <PlusOutlined />
        </button>
        <button type="button" onClick={() => handleRemoveOrigin(index)} className="delete-button">
          <DeleteOutlined />
        </button>
      </div>
    </fieldset>
